...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "db" do |db|
    ...
    db.vm.provision "shell", inline: <<-SHELL
      #Uppdatera ubuntu och förbereder för externa paketkällor
      apt-get update -y
      apt-get install -y software-properties-common

    # Lägger till ansibles officiella ppa-repository för att garantera att ansible kan installeras via apt-get.
